            ...
            for t in chunk:
                try:
                    ...
                    if (
                        ...
                        c > w_ma20 and v > (v20_a * 2.0) and 
                        day_ret >= 0.025 and v >= 2000000 
                    ):
...
# --- 4. 主程式分頁 ---
    ...
    with tab1:
        ...
        if 'scan_res' in st.session_state:
            ...
            for s in display_list:
                with st.container():
                    st.markdown(f"""<div class='stock-card'><h3>{s['代碼']} - {s['產業']}</h3><p>💰 目前價格: <span class='price-tag'>${s['現價']}</span> | 📊 成交量: {s['成交量']} 張</p><p>🛑 動態停損(20MA): {s['停損']} | 🎯 預設停利: {s['停利']}</p><a href='https://www.wantgoo.com/stock/{s['代碼']}' target='_blank'>📈 查看線圖</a></div>""", unsafe_allow_html=True)
                    with st.expander(f"🛒 買進 {s['代碼']}"):
                        # --- 修改：支援零股買進 (step=0.001) ---
                        qty = st.number_input("購買張數 (1張=1000股，零股請輸小數)", min_value=0.001, value=1.0, step=0.001, key=f"q_{s['代碼']}")
        ...
        if st.session_state.port:
            for tk, d in list(st.session_state.port.items()):
                try:
                    ...
                    live_ma20, live_ma60 = hist['Close'].rolling(20).mean().iloc[-1], hist['Close'].rolling(60).mean().iloc[-1]
                    ...
                    if now_p <= sl_val: st.error(f"⚠️ \"{stock_id}\" 低於停損位 {sl_val:.2f}")
                    color = "profit-up" if profit >= 0 else "profit-down"
                    st.markdown(f"""<div class='stock-card'><h4>{stock_id} ({round(d['q'], 3)} 張)</h4><p>損益: <span class='{color}'>${profit:,.0f}</span> ({profit_rate:.2f}%)</p><p>成本: {cost_per_share:.2f} | 現價: {now_p:.2f}</p></div>""", unsafe_allow_html=True)
                    
                    with st.expander(f"💸 賣出 {stock_id}"):
                        # --- 修改：支援零股賣出 (step=0.001) ---
                        ...
                        if st.button(f"執行賣出 {s_qty} 張", key=f"sbtn_{tk}"):
                            ...
                            st.session_state.history.append({"date": datetime.now().strftime("%Y-%m-%d %H:%M"), "month": datetime.now().strftime("%Y-%m"), "stock": stock_id, "qty": s_qty, "profit": realized_p, "pct": f"{realized_pct}%"})
    ...
    with tab3:
        if st.session_state.history:
            ...
            st.markdown(f"#### 💰 總已實現損益: ${view_df['profit'].sum():,.0f}")
            st.dataframe(view_df[['date', 'stock', 'qty', 'profit', 'pct']].sort_values('date', ascending=False), use_container_width=True)
        else: st.info("尚無紀錄")

    with tab4:
        tickers_map = get_all_tickers()
        selected_stock = st.selectbox("🔍 搜尋並加入股票代號", options=list(tickers_map.keys()), format_func=lambda x: tickers_map.get(x))
        if st.button("➕ 加入自選"):
        ...
        if st.session_state.get('watchlist'):
            for wt in st.session_state.watchlist:
                sid = wt.split('.')[0]
                st.markdown(f"<div class='stock-card'><h4>{tickers_map.get(wt)}</h4><a href='https://www.wantgoo.com/stock/{sid}' target='_blank'>📈 查看線圖</a></div>", unsafe_allow_html=True)
                if st.button(f"🗑️ 移除 {sid}", key=f"rem_{sid}"):
                    st.session_state.watchlist.remove(wt); supabase.table("users").update({"watchlist": st.session_state.watchlist}).eq("username", st.session_state.user).execute(); st.rerun()
