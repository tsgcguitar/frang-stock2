            ...
            for s in display_list:
                with st.container():
                    ...
                    with st.expander(f"🛒 買進 {s['代碼']}"):
                        # --- 修改點：允許零股輸入 (min_value=0.001, step=0.001) ---
                        qty = st.number_input("購買張數 (可輸入小數點，如 0.1)", min_value=0.001, value=1.0, step=0.001, key=f"q_{s['代碼']}")
                        total_cost = qty * 1000 * s['現價']
                        st.markdown(f"**預計買入總金額： `${total_cost:,.2f}`**")
                        if st.button(f"確認買進 {s['代碼']}", key=f"btn_{s['代碼']}"):
